...
const DoctorsComponent = ({ clinic }) => {
  ...
  const handleDoctorPress = () => {
    console.log(`Clicked on: ${clinic.name}`);
    navigation.navigate('ScrollableContainer', { clinicId: clinic.id });
  };
...
const Items = () => {
  ...
  useEffect(() => {
    const fetchClinics = async () => {
      try {
        const response = await fetch('https://zhs-backend.onrender.com/clinics');
